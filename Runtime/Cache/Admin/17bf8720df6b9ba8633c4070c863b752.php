...
<body>
    ...
    <div class="main-title">
        <h2><?php echo isset($info['id'])?'编辑':'新增';?>图文样式</h2>
    </div>
    <form action="<?php echo ($postUrl); ?>" method="post" class="form-horizontal">
        <div class="form-item">
            <label class="item-label">样式分组<span class="check-tips"></span></label>
            <div class="controls">
            	<select name="group_id">
            	<?php if(is_array($info['group_list'])): $i = 0; $__LIST__ = $info['group_list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option <?php if(($vo['selected']) == "1"): ?>selected="selected"<?php endif; ?> value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["group_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
        </div>
        <div class="form-item" style="clear:both; overflow:hidden">
        	<div class="fl">
            <label class="item-label">样式内容<span class="check-tips"></span></label>
            <div class="controls" style="width:600px" >
             <label class="textarea">
                <textarea style="height:200px" name="style" onKeyUp="$('.style_content').html($(this).val())" ><?php echo ($info["style"]); ?></textarea>
             </label>
            </div>
            </div>
