...
class SurveyController extends AddonsController {
	...
	public function survey(){
		...
		$msg["name"] = $name;
		$msg["phone"] = $phone;
		$msg["time"] = strftime($time);
		//$msg["aa"] = 12123;
		//$msg['abc'] = '32';
		//$msg['time'] = $time;
		//var_dump($msg);
		$insert = M ( 'survey_userlog' )->add ($msg);
		if ($insert){
		...
		/*if (!empty($data)){
			$insert = M ( 'survey_userlog' )->add ( $data );
			if($insert){
	...
	//	//$token = get_token();
	//	//$survey = D ('survey', )->getSurveyInfo($id);
	//	//$list = D('SurveyQuestion')->getQuestionInfo($info);
	//	if(IS_POST){
	//		$name = $_GET['name'];
	//		$tel = $_GET['tel'];
	//		$insert = I($name,$tel value 'name','tel');
	//		if($insert){
	//			$this->display('b.html');
	//			$this->succes('成功！')
	//		}else{
